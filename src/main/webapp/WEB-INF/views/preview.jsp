							...
							<c:forEach var="module" items="${list}">
								<li id="${module.id}" title="${module.title}"
									...
									class="module_nav_row"><a href="#"> <i
										class="glyphicon glyphicon-folder-open"></i> ${module.title } <span class="pull-right badge badge-success">${module.functions }</span>  
								</a></li>
							</c:forEach>
						</ul>
					</div>
...
<script type="text/javascript">
	...
	$(document).on("click", "#function_test_btn", function() {
		...
		window.open("../preview/test.do?id="+curFunction.id+"&proID="+${project.id});
